...
- BEの処理
    - index
        - ローカル変数のtasksにTask.allを代入する
            - Taskモデルを介して全てのレコードを代入する
                - json形式にして値を返す
    - create
        - Taskモデルにcreateメソッドを使ってインスタンスの生成から保存までを引数にparamsの値と初期を設定し実行する
        - headにはcreatedを格納して返す
    - destroy
        - ローカル変数のtaskに対してparamsから受け取ったidを元にfind関数を実行して代入する
        - レコードが代入された変数taskに対してdestroyメソッドを使って特定のレコードを削除する
        - headにはokを格納して返す
    - update
        - ローカル変数のtaskに対してfind関数を実行する
            - 引数にはリクエストパラメーターから受け取ったidを使ってレコードを検索する
            - 変数taskのis_doneプロパティにはparamsから受け取ったis_doneを代入する
            - 変数taskにsaveメソッドを実行してインスタンスをDBへ保存する
            - headにはokを格納する
    - edit
        - ローカル変数のtaskにfind関数を使ってリクエストパラメーターのidを受け取ってレコードを取得し代入する
            - 変数taskのnameプロパティにはリクエストパラメーターから受け取ったnameを代入する
            - taskをsaveメソッドを使ってDBへ保存する
            - headにはokを格納する